define([
  ...
  './object',
  './mixins/scheduler'
], function(_, $, RSVP, PObject, SchedulerMixin) {
  ...
  var View = PObject.extend(SchedulerMixin, {
    name: 'View',
    queues: [ 'render', 'remove' ],
    inherits: [ 'options' ],

    constructor: function() {
      PObject.call(this, 'view', function() {
        this.cid = _.uniqueId('view');
        this._ensureElement();
    ...
    _render: function(options) {
      ...
      RSVP.Promise.cast(that.load()).then(function() {
        return that.render(options, deferred);
      }).otherwise(function(error) {
    ...
    _remove: function(_deferred) {
      var deferred = _deferred || RSVP.defer();
      var that = this;
