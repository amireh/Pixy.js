  ...
  _.extend(History.prototype, Events, {
    ...
    start: function(options) {
      ...
      if (oldIE && this._wantsHashChange) {
        this.iframe = Pixy.$('<iframe src="javascript:0" tabindex="-1" />').hide().appendTo('body')[0].contentWindow;
      ...
      if (this._hasPushState) {
        Pixy.$(window).on('popstate', this.checkUrl);
      } else if (this._wantsHashChange && ('onhashchange' in window) && !oldIE) {
        Pixy.$(window).on('hashchange', this.checkUrl);
    ...
    stop: function() {
      Pixy.$(window).off('popstate', this.checkUrl).off('hashchange', this.checkUrl);
    ...
    loadUrl: function(fragmentOverride) {
      console.debug('Hash/URL changed:', fragmentOverride)
    ...
    navigate: function(fragment, options) {
      ...
      var out = this.__navigate.apply(this, arguments);
