define([
  ...
  './deep_model',
  './collection',
], function(_, Pixy, PixyObject, Model, DeepModel, Collection) {
  'use strict';
  ...
  return PixyObject.extend({
    ...
    initialize: function(options) {
      ...
      var ensureKlassName = function(klass, klassName) {
        if (Pixy[klass] && !Pixy[klass].prototype.klass) {
          Pixy[klass].prototype.klass = klassName || klass;
        }
      };

      var trackOverriddenMethods = function(klassName, cacheableName) {
        var methodId;
        var klass = Pixy[klassName].prototype;
      ...
      this.listenTo(Pixy, 'model:creating collection:creating', this.makeCacheable);
    },

    destroy: function() {
      _.each([ 'Model', 'Collection', 'DeepModel' ], function(klassName) {
        if (Pixy[klassName]) {
          delete Pixy[klassName].prototype._noCache;
          delete Pixy[klassName].prototype.klass;
        }
      });
    },
